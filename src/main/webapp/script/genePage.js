function loadSummaryMatrix(gene) {
	console.log(gene);
	...
	$.ajax({
		...
		success : function(data) {
			console.log(JSON.parse(data).summaryMatrix);
			make_d3_clustergram(JSON.parse(data).summaryMatrix);
		},
		error : function() {
			console.log('error');
		}
...
function make_d3_clustergram(network_data) {
	...
	clust_group = d3.select("#svg_div").append("svg").attr('id', 'main_svg')
	// the svg can be larger than the visualization - use svg_height and
	// svg_width
	.attr("width", svg_width + margin.left + margin.right + spillover_x_offset)
			.attr("height", svg_height + margin.top + margin.bottom).attr(
					'border', 1).call(zoom).append("g").attr('id',
					'clust_group').attr("transform",
					"translate(" + (margin.left) + "," + (margin.top) + ")");

	// grey background rect for clustergram
	d3.select('#clust_group').append("rect").attr("class", "background").attr(
			'id', 'grey_background').attr("width", svg_width).attr("height",
			viz_height);

	// make rows
	// use matrix for the data join, which contains a two dimensional
	// array of objects, each row of this matrix will be passed into the row
	// function
	var row_obj = clust_group.selectAll(".row").data(matrix).enter()
	...
	// horizontal lines
	row_obj.append('line').attr('x2', 20 * svg_width).style('stroke-width',
			border_width / zoom_switch + 'px')

	// append vertical line groups
	vert_lines = clust_group.selectAll('.vert_lines').data(col_nodes).enter()
			.append('g').attr('class', 'vert_lines').attr('transform',
					function(d, i) {
						return 'translate(' + x_scale(i) + ') rotate(-90)';
					})

	// add vertical lines
	vert_lines.append('line').attr('x1', 0).attr('x2', -20 * viz_height).style(
	...
	d3.select('#main_svg').append('rect').attr('fill', 'white').attr('width',
			row_label_width + 'px').attr('height', '3000px').attr('class',
			'white_bars');

	// append group for row labels
	d3.select('#main_svg').append("g").attr('id', 'row_labels').attr(
			"transform",
			"translate(" + row_margin.left + "," + row_margin.top + ")")

	// generate and position the row labels
	var row_label_obj = d3.select('#row_labels').selectAll('.row_label_text')
			.data(row_nodes).enter().append('g')
			.attr('class', 'row_label_text').attr('transform', function(d, i) {
				return "translate(0," + y_scale(i) + ")";
			}).on('click', reorder_click_row).on('mouseover', function() {
				// highlight text
				d3.select(this).select('text').style('font-weight', 'bold');
			}).on("mouseout", function mouseout() {
				d3.select(this).select('text').style('font-weight', 'normal');
				// reset highlighted col
				d3.select('#clicked_row').style('font-weight', 'bold');
			});
	...
	// !! can be improved
	.attr('y', y_scale.rangeBand() / 2).attr('dy', y_scale.rangeBand() / 4)
			.attr('text-anchor', 'end').style('font-size',
					default_fs_row + 'px').text(function(d, i) {
				return d.name;
			})

	// append rectangle behind text
	row_label_obj.insert('rect', 'text').attr('x', -10).attr('y', 0).attr(
			'width', 10).attr('height', 10).style('opacity', 0);

	// change the size of the highlighting rects
	row_label_obj.each(function() {
		...
		d3.select(this).select('rect').attr('x', bbox.x * 0.5).attr('y', 0)
				.attr('width', bbox.width * 0.5).attr('height',
						y_scale.rangeBand()).style('fill', 'yellow').style(
						'opacity', 0);
	});
	...
	d3.select('#main_svg').append('rect').attr('fill', 'white').attr('height',
			col_label_width + 'px').attr('width', '3000px').attr('class',
			'white_bars');

	// append group for column labels
	d3.select('#main_svg').append("g").attr('id', 'col_labels').attr(
			"transform",
			"translate(" + col_margin.left + "," + col_margin.top + ")");
	...
	col_label_obj = d3.select('#col_labels').selectAll(".col_label_text").data(
			col_nodes).enter().append("g").attr("class", "col_label_text")
	...
	.attr(
			'transform',
			'translate(' + x_scale.rangeBand() / 2 + ',' + x_offset_click
					+ ') rotate(45)').on('click', reorder_click_col).on(
			'mouseover', function() {
				// highlight text
				d3.select(this).select('text').style('font-weight', 'bold');
			}).on("mouseout", function mouseout() {
	...
	// add column label
	col_label_click.append("text").attr("x", 0).attr("y",
			x_scale.rangeBand() / 2).attr('dx', 2 * border_width)
	// .attr("dy", ".32em")
	.attr("text-anchor", "start").attr('full_name', function(d) {
		return d.name
	...
	// append rectangle behind text
	col_label_click.insert('rect', 'text').attr('x', 10).attr('y', 0).attr(
			'width', 10).attr('height', 10).style('opacity', 0);
	...
	col_label_click.each(function() {
		...
		// use the bounding box to set the size of the rect
		d3.select(this).select('rect').attr('x', bbox.x * 1.25).attr('y', 0)
	...
	col_label_click
			...
			.attr(
					...
					function(d) {
						...
						output_string = 'M ' + origin_y + ',0 L ' + start_y
								+ ',' + start_x + ', L ' + final_y + ','
								+ final_x + ' Z';
	...
	d3.select('#main_svg').append('rect').attr('fill', 'white').attr('width',
			row_label_width + 'px').attr('height', col_label_width + 'px')
			.attr('id', 'top_left_white');

	// hide spillover from right
	d3.select('#main_svg').append('rect').attr('fill', 'white').attr('width',
			'200px').attr('height', '3000px').attr('transform', function() {
	...
	d3.select('#main_svg').append('path').style('stroke-width', '0')
	// mini-language for drawing path in d3, used to draw triangle
	.attr('d', 'M 0,0 L 500,-500, L 500,0 Z').attr('fill', 'white').attr('id',
			'slant_traingle').attr('transform', function() {
		tmp_left = (margin.left + svg_width);
...
function row_function(row_data) {
	...
	cell = d3.select(this)
	// data join
	.selectAll(".cell").data(row_data).enter().append("rect").attr('class',
			'cell').attr("x", function(d) {
		return x_scale(d.pos_x);
	}).attr("width", x_scale.rangeBand()).attr("height", y_scale.rangeBand())
			...
			.style('fill', function(d) {
				...
				// return inst_color ;
					...
					"mouseover",
					function(p) {
						d3.selectAll(".row_label_text text").classed("active",
								function(d, i) {
									return i == p.pos_y;
								});
						d3.selectAll(".col_label_text text").classed("active",
								function(d, i) {
									return i == p.pos_x;
								});
					}).on("mouseout", function mouseout() {
...
function reorder_clust_rank(order_type) {
	...
	t.selectAll(".row").attr("transform", function(d, i) {
		return "translate(0," + y_scale(i) + ")";
	}).selectAll(".cell").attr('x', function(d) {
		return x_scale(d.pos_x);
	})

	// Move Row Labels
	d3.select('#row_labels').selectAll('.row_label_text').transition()
			.duration(2500).attr('transform', function(d, i) {
				return 'translate(0,' + y_scale(i) + ')';
			});

	// Move Col Labels
	d3.select('#col_labels').selectAll(".col_label_text").transition()
			.duration(2500).attr("transform", function(d, i) {
				return "translate(" + x_scale(i) + ")rotate(-90)";
			})
...
function initialize_clustergram(network_data) {
	...
	scale_reduce_font_size_factor = d3.scale.log().domain(
			[ min_node_num, max_node_num ]).range([ min_fs_zoom, max_fs_zoom ])
			.clamp('true');
	// // define the scaling for the zoomability of the adjacency matrix
	// scale_zoom =
	// d3.scale.log().domain([min_node_num,max_node_num]).range([2,17]).clamp('true');
	...
	scale_fs_screen_width = d3.scale.linear().domain(
			[ min_viz_width, max_viz_width ]).range([ 0.75, 1.15 ]).clamp(
			'true');
	scale_fs_screen_height = d3.scale.linear().domain(
			[ min_viz_width, max_viz_width ]).range([ 0.75, 1.15 ]).clamp(
			'true');

	// the default font sizes are set here
	default_fs_row = scale_font_size(row_nodes.length)
			* scale_fs_screen_height(viz_height);
	// the colum font size is scaled by the width
	default_fs_col = scale_font_size(col_nodes.length)
			* scale_fs_screen_width(viz_width);

	// correct for forcing the tiles to be squares - if they are forced, then
	// use the col font size scaling on the rows
	if (force_square == 1) {
	...
	// calculate the reduce font-size factor: 0 for no reduction in font size
	// and 1 for full reduction of font size
	...
	real_zoom_scale_col = d3.scale.linear().domain(
			[ min_node_num, max_node_num ]).range([ 2, 5 ]).clamp('true');
	// scale the zoom based on the screen size
	// smaller screens can zoom in more, compensates for reduced font size with
	// small screen
	real_zoom_scale_screen = d3.scale.linear().domain(
			[ min_screen_width, max_screen_width ]).range([ 2, 1 ]).clamp(
			'true');
	// calculate the zoom factor - the more nodes the more zooming allowed
	real_zoom = real_zoom_scale_col(col_nodes.length)
			* real_zoom_scale_screen(screen_width);

	// set opacity scale
	max_link = _.max(inst_links, function(d) {
		return Math.abs(d.value)
	})
	opacity_scale = d3.scale.linear().domain([ 0, Math.abs(max_link.value) ])
			.clamp(true).range([ 0.0, 1.0 ]);
};

function set_visualization_size() {

	// find the label with the most characters and use it to adjust the row and
	// col margins
	row_max_char = _.max(row_nodes, function(inst) {
		return inst.name.length;
	}).name.length;
	col_max_char = _.max(col_nodes, function(inst) {
		return inst.name.length;
	}).name.length;

	// define label scale parameters: the more characters in the longest name,
	// the larger the margin
	min_num_char = 5;
	max_num_char = 40;
	min_label_width = 60;
	max_label_width = 200;
	label_scale = d3.scale.linear().domain([ min_num_char, max_num_char ])
	...
	// Margins
	col_margin = {
		top : col_label_width - label_margin,
		right : 0,
		bottom : 0,
	...
	row_margin = {
		...
		right : 0,
		bottom : 0,
		left : row_label_width - label_margin
	};
	margin = {
		top : col_label_width,
		right : 0,
	...
	// 
	// get screen width
	screen_width = Number(d3.select('#wrapper').style('width')
			.replace('px', ''));
	// get screen height
	...
	// reducing sidebar width, row_offset used to be 280 when left margin was
	// 260
	...
	d3.select('#clustergram_container').style('width',
			width_clust_container + 'px')
	d3.select('#clustergram_container').style('height',
			height_clust_container + 'px')
	// clust_and_row_container
	d3.select('#clust_and_row_container').style('width',
			width_clust_container + 'px')
	d3.select('#clust_and_row_container').style('height',
			height_clust_container + 'px')
	...
	// subtract fixed length for permanent col and row labels and variable
	// length for specific col and row labels
	...
	prevent_col_stetch = d3.scale.linear().domain([ 1, 20 ]).range([ 0.05, 1 ])
			.clamp('true');
	...
	if (col_nodes.length > row_nodes.length) {
		...
		if (viz_height > svg_height) {
			// make the height equal to the width, to force square tiles -
			// rather than thin tiles that
	...
	orders = {
		...
		clust_row : d3.range(col_nodes.length).sort(function(a, b) {
			return col_nodes[b].sort - col_nodes[a].sort;
		}),
		clust_col : d3.range(row_nodes.length).sort(function(a, b) {
			return row_nodes[b].sort - row_nodes[a].sort;
		})
	...
	zoom_switch = (viz_width / col_nodes.length)
			/ (viz_height / row_nodes.length);
};
...
function apply_transformation(trans_x, trans_y, zoom_x, zoom_y, duration) {
	...
	// if the transformation is from a gene search, the remove pan_room_y
	// restriction
	if (duration > 0) {
	...
	clust_group.attr('transform', 'translate('
			+ [ margin.left + trans_x, margin.top + trans_y ] + ') scale('
			+ zoom_x + ',' + zoom_y + ')');

	// transform row labels
	d3.select('#row_labels').attr(
			'transform',
			'translate(' + [ row_margin.left, margin.top + trans_y ]
					+ ') scale(' + zoom_y + ')');

	// transform col labels
	// move down col labels as zooming occurs, subtract trans_x - 20 almost
	// works
	d3.select('#col_labels').attr(
			'transform',
			'translate(' + [ col_margin.left + trans_x, col_margin.top ]
					+ ') scale(' + zoom_x + ')');
	...
	// if reduce_font_size_factor_ is 1, then the font will be divided by the
	// whole zoom - and the labels will not increase in size
	// if reduce_font_size_factor_ is 0, then the font will be divided 1 - and
	// the labels will increase cuction of the font size
	reduce_font_size = d3.scale.linear().domain([ 0, 1 ]).range([ 1, zoom_y ])
			.clamp('true');
	...
	d3.select('#row_labels').each(
			function() {
				// get the bounding box of the row label text
				var bbox = d3.select(this).select('text')[0][0].getBBox();

				// use the bounding box to set the size of the rect
				d3.select(this).select('rect').attr('x', bbox.x * 0.5).attr(
						'y', 0).attr('width', bbox.width * 0.5).attr('height',
						y_scale.rangeBand()).style('fill', 'yellow');
			});

	// reduce font-size to compensate for zoom
	// calculate the recuction of the font size
	reduce_font_size = d3.scale.linear().domain([ 0, 1 ]).range([ 1, zoom_x ])
			.clamp('true');
	// scale down the font to compensate for zooming
	...
	d3.select('#col_labels').each(
			function() {
				...
				// use the bounding box to set the size of the rect
				d3.select(this).select('rect').attr('x', bbox.x * 1.25).attr(
						'y', 0).attr('width', bbox.width * 1.25)
				// used teh reduced rect width for the columsn
				// reduced because thee rects are slanted
				.attr('height', x_scale.rangeBand() * 0.6).style('fill',
...
function reorder_click_row(d, i) {
	...
	d3.selectAll('.rol_label_text').select('text').style('font-weight',
			'normal');
	// remove previous id
	d3.select('#clicked_row').attr('id', '');

	// highlight current
	d3.select(this).select('text').style('font-weight', 'bold').attr('id',
			'clicked_row');
	...
	// sort the rows
	tmp_sort = d3.range(tmp_arr.length).sort(function(a, b) {
...
function reorder_click_col(d, i) {
	...
	// highlight clicked column
	// first un-highlight all others
	d3.selectAll('.col_label_text').select('text').style('font-weight',
			'normal');
	// remove previous id
	...
	d3.selectAll('.col_label_text').select('rect').style('opacity', 0);
...
function two_translate_zoom(pan_dx, pan_dy, fin_zoom) {
	...
	if (pan_dy >= half_height - y_pan_room) {
		...
		// if the amount of panning is equal to the half_height then it needs to
		// be reduced
		// effectively, the the visualization needs to be moved up (negative) by
		// some factor
		// of the half width of the visualization. If there was no zooming
		// involved, then the
		// visualization would be centered first, then panned to center the top
		// term, then
		// the correction would re-center it. However, because of the zooming
		// the offset is
		// reduced by the zoom factor (this is because the panning is occurring
		// on something
		// that will be zoomed into - this is why the pan_dy value is not scaled
		// in the two
		// translate transformations, but it has to be scaled afterwards to set
		// the translate
		// vector)
		// pan_dy = half_height - (half_height)/zoom_switch

		// if pan_dy is greater than the pan room, then panning has to be
		// restricted
		// start by shifting back up (negative) by half_height/zoom_switch then
		// shift bak down
		// by the difference between half_height and pan_dy (so that the top of
		// the clustergram is
		// visible)
		var shift_top_viz = half_height - pan_dy;
		var shift_up_viz = -half_height / zoom_switch + shift_top_viz;

		// reduce pan_dy so that the visualization does not get panned to far
		// down
	...
	clust_group.transition().duration(search_duration)
	// first apply the margin transformation
	// then zoom, then apply the final transformation
	.attr(
			'transform',
			'translate(' + [ margin.left, margin.top + center_y ] + ')'
					+ ' scale(' + 1 + ',' + zoom_y + ')' + 'translate('
					+ [ pan_dx, pan_dy ] + ')');

	// transform row labels
	d3.select('#row_labels').transition().duration(search_duration).attr(
			'transform',
			'translate(' + [ row_margin.left, margin.top + center_y ] + ')'
					+ ' scale(' + zoom_y + ',' + zoom_y + ')' + 'translate('
					+ [ 0, pan_dy ] + ')');

	// transform col labels
	// move down col labels as zooming occurs, subtract trans_x - 20 almost
	// works
	d3.select('#col_labels').transition().duration(search_duration).attr(
			'transform',
			'translate(' + [ col_margin.left, col_margin.top ] + ')'
					+ ' scale(' + 1 + ',' + 1 + ')' + 'translate('
					+ [ pan_dx, 0 ] + ')');
	...
	fin_font = fin_font + 'px';
	// change the font size of the labels
	d3.selectAll('.row_label_text').transition().duration(search_duration)
			.select('text').style('font-size', fin_font);
	...
	fin_font = fin_font + 'px';
	// change the font size of the labels
	d3.selectAll('.col_label_text').transition().duration(search_duration)
			.select('text').style('font-size', fin_font);

	// set y translate: center_y is positive, positive moves the visualization
	// down
	// the translate vector has the initial margin, the first y centering, and
	// pan_dy
...
function zoom_and_highlight_found_gene(search_gene) {
	...
	d3.selectAll('.row_label_text').select('text').style('font-weight',
			'normal');
