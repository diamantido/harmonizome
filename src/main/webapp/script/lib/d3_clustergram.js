...
function make_d3_clustergram(network_data, d3_parentEl) {

	// Move this to outside the function.
	if (svg) {
		svg.remove();
	}
	svg = d3_parentEl.append("svg");
	...
	setVisualizationSize(svg, col_nodes, row_nodes);
	initializeVariables(col_nodes, row_nodes, inst_links, d3_parentEl);
	...
	// Transfer network information to matrix.
 ...
 */
function initializeVariables(col_nodes, row_nodes, inst_links, d3_parentEl) {
  
	// define global reordering value 
	// not currently reordering 
	global_reorder = false;

  	var parentDimensions = d3_parentEl.node().getBoundingClientRect();
  ...
  //set_visualization_size(col_nodes, row_nodes, d3_parentEl);
...
function createLabels(svg, colNodes, rowNodes) {

		// Find the label with the most characters and use it to adjust the row
		// and col margins 
	var ROW_MAX_CHARS = _.max(rowNodes, function(inst) { return inst.name.length; }).name.length,
		COL_MAX_CHARS = _.max(colNodes, function(inst) { return inst.name.length; }).name.length,
		// Account for the triangular space.
		TRIANGLE_SPACE = 30,
		// Define label scale parameters; the more characters in the longest
		// name, the larger the margin.
		MIN_NUM_CHARS = 5,
		MAX_NUM_CHARS = 40,
		MIN_LABEL_WIDTH = 60,
		MAX_LABEL_WIDTH = 200;
	...
	row_label_width = labelScale(ROW_MAX_CHARS) ;
	col_label_width = labelScale(COL_MAX_CHARS) + TRIANGLE_SPACE;
	...
	// TODO: Ask Nick what this does.
	spillover_x_offset = labelScale(COL_MAX_CHARS) * 0.8;
};


function setVisualizationSize(svg, col_nodes, row_nodes, d3_parentEl) {
	
	createLabels(svg, col_nodes, row_nodes);

  	SCREEN_WIDTH  = $('body').width();
  	SCREEN_HEIGHT = $('body').width();

  	// Define offsets for permanent row and col margins, taking the sidebar
  	// into consideration.
  	var CONTAINER_ROW_OFFSET = 230,
